...
export function findCorrections(text, charts) {
  ...
  for (const notice of notices) {
    for (const chart of charts) {
      ...
      // Always run this (even with directMatch) to collect ALL relevant
      // panel numbers for excerpt extraction.
      ...
      // But 5614_3 "Approaches to Lowestoft" does NOT match — only exact panels.
      const relevantNums = new Set([chartStr]);
      ...
      if (directMatch || panelMatch) {
        ...
        // Extract chart sections that match the folio chart or its panels.
        // Notices can have interleaved charts (e.g., 1534, 1535, 1543, 5614_4)
        // so we collect each matching section individually and join them.
        const header = cleanText.split("\n").slice(0, 2).join("\n");
        const sections = [];

        // Find every "Chart NNNN" or "Chart NNNN_N" boundary in the notice
        const chartBoundaryRegex =
          /\nChart\s+(\d{3,5}(?:_\d+)?)(?:\s|\(|\[)/gi;
        const boundaries = [];
        let bm;
        while ((bm = chartBoundaryRegex.exec(cleanText)) !== null) {
          boundaries.push({ index: bm.index, num: bm[1] });
        }

        for (let bi = 0; bi < boundaries.length; bi++) {
          // Match exact chart number or folio chart as base (Chart 1534_1 matches "1534")
          const num = boundaries[bi].num;
          const base = num.split("_")[0];
          if (relevantNums.has(num) || relevantNums.has(base)) {
