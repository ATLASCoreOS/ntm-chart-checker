  ...
  for (const notice of notices) {
    for (const chart of charts) {
      ...
      if (directMatch || panelMatch) {
        ...
        // Build regex alternation for all relevant chart bases
        // e.g., for chart 1534 with panel 5614_4: "1534(?:_\\d+)?|5614(?:_\\d+)?"
        const basesAlt = [...relevantBases]
          .map((b) => `${b}(?:_\\d+)?`)
          .join("|");
        ...
        const chartPos = cleanText.search(
          new RegExp(`Chart\\s+(?:${basesAlt})(?:\\s|\\(|\\[)`, "i")
        );
        if (chartPos >= 0) {
          const header = cleanText.split("\n").slice(0, 2).join("\n");
          const rest = cleanText.substring(chartPos);
          // Find next chart section with a base NOT in our relevant set
          const nextChartMatch = rest.search(
            new RegExp(
              `\\nChart\\s+(?!(?:${basesAlt})(?:\\s|\\(|\\[))\\d{3,5}(?:_\\d+)?(?:\\s|\\(|\\[)`,
              "i"
            )
          );
          const chartSection =
