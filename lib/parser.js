...
export function findCorrections(text, charts) {
  ...
  for (const notice of notices) {
    for (const chart of charts) {
      ...
      // Pattern 3: Panel name match — scan for panel chart references
      // whose description matches this folio chart's geographic name.
      // e.g., "Chart 5614_4 (Panel A, Approaches to Great Yarmouth)"
      // matches folio chart 1534 "Great Yarmouth and Approaches"
      let panelBase = null;
      if (!directMatch && chartCores[chart]) {
        const panelRefRegex =
          /Chart\s+(\d{3,5}_\d+)\s*\((?:Panel\s+[A-Z],?\s*)?([^)]+)\)/gi;
        let m;
        while ((m = panelRefRegex.exec(notice.text)) !== null) {
          const panelDesc = m[2].trim();
          const panelCore = extractCore(panelDesc).toLowerCase();
          if (panelCore.length >= 5 && panelCore === chartCores[chart]) {
            panelBase = m[1].split("_")[0];
            break;
          }
        }
      }

      if (directMatch || panelBase) {
        // Clean up the text
        const cleanText = notice.text
          .replace(/Wk\d{2}\/\d{2}\s*\n\s*II\s*\n[\d\s.]+\n/g, "")
          .trim();

        // Use the direct chart number or matched panel base for excerpt extraction
        const searchNum = directMatch ? chartStr : panelBase;
