...
export function findCorrections(text, charts) {
  ...
  for (const notice of notices) {
    for (const chart of charts) {
      // Chart numbers may include underscore panel suffixes: 5614_4, 5614_3
      // Allow optional _N suffix so chart 5614 matches "Chart 5614_4"
      ...
      if (permPattern.test(notice.text) || affectedPattern.test(notice.text)) {
        ...
        // From "Chart NNNN ..." up to the next chart with a DIFFERENT base number
        let excerpt;
        const chartPos = cleanText.search(
          new RegExp(`Chart\\s+${chartStr}(?:_\\d+)?(?:\\s|\\(|\\[)`, "i")
        );
        if (chartPos >= 0) {
          const header = cleanText.split("\n").slice(0, 2).join("\n");
          const rest = cleanText.substring(chartPos);
          // Find where the next chart section begins (different base chart number)
          // Excludes panels of the same chart: 5614_3 when current is 5614
          const nextChartMatch = rest.search(
            new RegExp(
              `\\nChart\\s+(?!${chartStr}(?:_\\d+)?(?:\\s|\\(|\\[))\\d{3,5}(?:_\\d+)?(?:\\s|\\(|\\[)`,
              "i"
            )
          );
