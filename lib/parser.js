...
function extractSectionII(text) {
  // Find actual notices: NM number followed by spaces then ALLCAPS country/region.
  // Pattern: "759*  ENGLAND" or "770   CANADA" or "762(T)/26     WALES".
  // NB: \s{2,} (not 3) — the trailing "*" on starred notices consumes a column,
  // leaving only 2 spaces; \s{3,} silently dropped every starred notice.
  const noticeStartPattern =
    /\n(\d{3,5})(?:\*|\([TP]\)\/\d{2,4})?\s{2,}[A-Z]{2,}/;
  ...
  const realNoticePattern =
    /\n(\d{3,5})(?:\*|\([TP]\)\/\d{2,4})?\s{2,}[A-Z][A-Z]+\s*-\s/g;
...
function splitIntoNotices(sectionText) {
  ...
  const headerRegex =
    /\n(\d{3,5})(\([TP]\)\/\d{2,4}|\*)?\s{2,}([A-Z][A-Z\s]+?\s*-\s)/g;
  const headers = [];
  let match;
...
export function extractSectionIA(text) {
  const sectionIIStart =
    /\n(\d{3,5})(?:\*|\([TP]\)\/\d{2,4})?\s{2,}[A-Z][A-Z]+\s*-\s/.exec(text);
...
export function findTPNotices(text, charts) {
  ...
  const tpHeaderRegex =
    /\n(\d{3,5}\([TP]\)\/\d{2,4})\s{2,}([A-Z][A-Z\s]+?\s*-\s[^\n]+)/g;
  let match;
