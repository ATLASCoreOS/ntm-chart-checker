...
import chartPanelsMap from "./chart-panels.json";
...
export function findCorrections(text, charts) {
  ...
  for (const notice of notices) {
    for (const chart of charts) {
      ...
      // Associated panel chart base numbers from CAL mapping
      // e.g., chart 1534 → panels ["5614_4","5614_5"] → bases ["5614"]
      const panelNums = chartPanelsMap[chartStr] || [];
      const panelBases = [...new Set(panelNums.map((p) => p.split("_")[0]))];

      // Search for direct chart match first, then associated panel bases
      // Direct match takes priority for excerpt extraction
      const searchNums = [chartStr, ...panelBases];

      let matched = false;
      for (const searchNum of searchNums) {
        const isDirect = searchNum === chartStr;

        // Pattern 1: Permanent correction - "Chart  NNNN" or "Chart NNNN_N"
        const permPattern = new RegExp(
          `\\bChart\\s+${searchNum}(?:_\\d+)?(?:\\s|\\(|\\[)`,
          "i"
        );

        // Pattern 2: T&P affected charts (only for direct chart match)
        const affectedPattern = isDirect
          ? new RegExp(
              `Charts?\\s+affected\\s*-[\\s\\d\\-(,)INT_]*\\b${searchNum}\\b`,
              "i"
            )
          : null;

        if (
          permPattern.test(notice.text) ||
          (affectedPattern && affectedPattern.test(notice.text))
        ) {
          // Clean up the text
          ...
          // Extract chart-specific section from the notice
          let excerpt;
          const chartPos = cleanText.search(
            new RegExp(
              `Chart\\s+${searchNum}(?:_\\d+)?(?:\\s|\\(|\\[)`,
              "i"
            )
          );
          if (chartPos >= 0) {
            ...
            // Find next chart section with a DIFFERENT base number
            const nextChartMatch = rest.search(
              new RegExp(
                `\\nChart\\s+(?!${searchNum}(?:_\\d+)?(?:\\s|\\(|\\[))\\d{3,5}(?:_\\d+)?(?:\\s|\\(|\\[)`,
                "i"
              )
            );
            const chartSection =
          ...
          matched = true;
          break; // First match wins (direct before panels)
        }
      }
    }
  }
