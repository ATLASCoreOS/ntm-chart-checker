...
export function findTPInForce(text, charts) {
  ...
  // The T&P In Force list is only published monthly (not every week).
  // It uses dot-separated entries like: "4697(T)/25   1491, 2693......ENGLAND, East Coast: Buoyage.."
  // Check if dot-separated T&P entries exist in Section IA.
  const hasDotEntries = /\d{3,5}\([TP]\)\/\d{2,4}\s+[\d,\s_A-Za-z]+?\.{3,}/.test(sectionIA);
  if (!hasDotEntries) {
    // T&P In Force list not published this week — return empty with flag
    tpInForce._listAvailable = false;
    return tpInForce;
  }
  tpInForce._listAvailable = true;
