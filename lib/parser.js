  ...
  for (const notice of notices) {
    for (const chart of charts) {
      ...
      if (permPattern.test(notice.text) || affectedPattern.test(notice.text)) {
        ...
        // Build a chart-specific excerpt by finding the "Chart  NNNN" line
        let excerpt;
        const chartPos = cleanText.search(
          new RegExp(`Chart\\s+${chart}\\b`, "i")
        );
        if (chartPos >= 0) {
          // Include the notice header + chart-specific section
          const header = cleanText.split("\n").slice(0, 2).join("\n");
          excerpt = header + "\n" + cleanText.substring(chartPos, chartPos + 400);
        } else {
          excerpt = cleanText.slice(0, 600);
        }
        excerpt = excerpt.trim().slice(0, 600);
